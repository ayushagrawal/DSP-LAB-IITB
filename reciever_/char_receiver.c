...
unsigned short data_received = 0;
unsigned short header = 0x001A; //header content of data packet
...
{ 
	...
	Int16 data_count = 1;
		...
		while(TRUE)
		{
			...
			//96/2=48 , 48 + 192 = 240, ...
			if( data_count == 48 || data_count == 240 || data_count == 512 || data_count == 624 || data_count == 816 || data_count == 1008 ){
				if( LEFT_DATA < -2500 ){
 ...
 * ------------------------------------------------------------------------ */ 
void check_pattern() {
	if(data_received >> 8 == header){
		printf("%c", (data_received & 0x00FF));
		data_received = 0;
		return;
	}
}						   			 
    ...
    while(TRUE)
    {
	    ...
	    if( LEFT_DATA > 2500 )
	    {
	    	...
	    	if( count > 3 )
	    	{
	    		...
	    		data_captured[data_cap_pointer] = 0;
	    ...
	    if( LEFT_DATA < -2500 )
	    {
	    	...
	    	if( count > 3 )
	    	{
	    		...
	    		data_captured[data_cap_pointer] = 1;
			    data_cap_pointer = ((data_cap_pointer + 1)%1024); 
	    		//printf("bit %d: %d\n", i, data_received[i]); 
